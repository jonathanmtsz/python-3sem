...
class Treinador:
    ...
    def adicionar_pokemon(self, apelido, tipo, experiencia):
        if apelido in self.__pokemons:
            raise PokemonJaExisteException()
...
def detalhar_pokemon(nome, apelido):
    if nome not in treinadores:
        return "Treinador não existe.", 404
    treinador = treinadores[nome]
    pokemons = treinador.pokemons
    if apelido not in pokemons:
        return "Pokémon não existe.", 404
...
def cadastrar_treinador(nome):
    if nome in treinadores:
        return jsonify(to_dict(treinadores[nome])), 303
    t = Treinador(nome)
    treinadores[nome] = t
...
def cadastrar_pokemon(nome, apelido):
    ...
    pokemon = treinador.adicionar_pokemon(
        apelido, dados['tipo'], dados['experiencia'])

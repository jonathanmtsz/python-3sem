...
class TestStringMethods(unittest.TestCase):
    ...
    def test_000_leciona(self):
        r = requests.get(f'http://{localhost}:5000/leciona/1/1/')
        ...
        r = requests.get(f'http://{localhost}:5000/leciona/2/1/')
        self.assertEqual(r.json(),{'leciona':False,'isok':True})
        r = requests.get(f'http://{localhost}:5000/leciona/2/3/')
        self.assertEqual(r.json(),{'leciona':True,'isok':True})

    
    def test_001a_leciona_not_found(self):
        r = requests.get(f'http://{localhost}:5000/leciona/1/100/')
        self.assertEqual(r.status_code,404)
    
    def test_001b_leciona_not_found(self):
        r = requests.get(f'http://{localhost}:5000/leciona/1/200/')
        self.assertEqual(r.status_code,404)
    ...
    def test_003_atividade(self):
        r = requests.get(f'http://{localhost}:5050/atividade/1/?id_professor=1')
        self.assertEqual(r.status_code,200)
        self.assertEqual(r.json()['atividade']['enunciado'], "crie um app de todo em flask")
        self.assertEqual(r.json()['isok'], True)
        r = requests.get(f'http://{localhost}:5050/atividade/2/?id_professor=3')
    ...
    def test_004_atividade_not_found(self):
        r = requests.get(f'http://{localhost}:5050/atividade/123/?id_professor=2')
        self.assertEqual(r.json()['isok'], False)
        self.assertEqual(r.status_code,404)
    ...
    def test_005b_atividade_url(self):
        ...
        r = requests.get(f'http://{localhost}:5050/atividades/ver_tudo/')
    ...
    def test_006a_professor_que_nao_leciona_nao_tem_acesso_as_respostas(self):
        r = requests.get(f'http://{localhost}:5050/atividade/1/professor/100')
        self.assertFalse('respostas' in r.json()['atividade'])


    
    def test_006b_professor_que_leciona_tem_acesso_as_respostas(self):
        r = requests.get(f'http://{localhost}:5050/atividade/1/professor/1')
    ...
    def test_007_request_sem_professor_volta_sem_as_respostas(self):
        r = requests.get(f'http://{localhost}:5050/atividade/1/')
        self.assertFalse('respostas' in r.json()['atividade'])

    def test_008a_alunos_e_notas(self):
        r = requests.get(f'http://{localhost}:5050/notas/janaina/1/')
        notas = r.json()['notas']
        self.assertEqual(notas, [0,0])
        r = requests.get(f'http://{localhost}:5050/notas/cicero/1/')
        notas = r.json()['notas']
    ...
    def test_008b_alunos_e_notas(self):
        r = requests.get(f'http://{localhost}:5050/notas/miguel/1/')
        notas = r.json()['notas']
        self.assertEqual(notas, [0,0])
        self.assertEqual(r.json()['isok'], True)

    def test_008c_alunos_e_notas_aluno_inexistente(self):
        r = requests.get(f'http://{localhost}:5050/notas/diana/1/')
        self.assertEqual(r.json()['isok'], False)
        self.assertEqual(r.status_code,404)
